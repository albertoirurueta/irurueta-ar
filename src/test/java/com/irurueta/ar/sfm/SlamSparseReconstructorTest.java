...
class SlamSparseReconstructorTest {
    ...
    private static final Logger LOGGER = Logger.getLogger(SlamSparseReconstructorTest.class.getSimpleName());
    ...
    void testConstructor() {
        assertEquals(2, SlamSparseReconstructor.MIN_NUMBER_OF_VIEWS);

        final var configuration = new SlamSparseReconstructorConfiguration();
        final var listener = new SlamSparseReconstructorListener() {
            @Override
            ...
            @Override
            public void onSlamCameraEstimated(final SlamSparseReconstructor reconstructor, final PinholeCamera camera) {
                // no action needed
            }

            @Override
            public boolean hasMoreViewsAvailable(final SlamSparseReconstructor reconstructor) {
                return false;
            }
        ...
        var reconstructor = new SlamSparseReconstructor(listener);
        ...
        for (var t = 0; t < TIMES; t++) {
            ...
            final var listener = new SlamSparseReconstructorListener() {
                ...
                public void onFundamentalMatrixEstimated(
                        final SlamSparseReconstructor reconstructor,
                        final EstimatedFundamentalMatrix estimatedFundamentalMatrix) {
                    SlamSparseReconstructorTest.this.estimatedFundamentalMatrix = estimatedFundamentalMatrix;
                }
                ...
                public void onEuclideanCameraEstimated(
                    ...
                    SlamSparseReconstructorTest.this.scale = scale;
                }
            ...
            final var reconstructor = new SlamSparseReconstructor(configuration, listener);
            ...
            double scaleRelativeError = Math.abs(baseline / scale - 1.0);
        ...
        for (var t = 0; t < TIMES; t++) {
            ...
            final var listener = new SlamSparseReconstructorListener() {
                ...
                public void onFundamentalMatrixEstimated(
                        final SlamSparseReconstructor reconstructor,
                        final EstimatedFundamentalMatrix estimatedFundamentalMatrix) {
                    if (SlamSparseReconstructorTest.this.estimatedFundamentalMatrix == null) {
                        SlamSparseReconstructorTest.this.estimatedFundamentalMatrix = estimatedFundamentalMatrix;
                ...
                @Override
                public void onMetricCameraEstimated(
                        final SlamSparseReconstructor reconstructor, final int previousViewId, final int currentViewId,
                        final EstimatedCamera previousCamera, final EstimatedCamera currentCamera) {
                ...
                @Override
                public void onMetricReconstructedPointsEstimated(
                        final SlamSparseReconstructor reconstructor, final List<MatchedSamples> matches,
                        final List<ReconstructedPoint3D> points) {
                    metricReconstructedPoints = points;
                }
    ...
    void testGeneralPointsEssentialWithConstantAccelerationAndRotationWithNoiseThreeViews()
        ...
        for (var t = 0; t < TIMES; t++) {
            ...
            final var estEuclideanCam1 = this.estimatedEuclideanCamera1.getCamera();
            final var estdEuclideanCam2 = this.estimatedEuclideanCamera2.getCamera();
            final var estEuclideanCam3 = this.estimatedEuclideanCamera3.getCamera();
            ...
            for (var i = 0; i < numReconstructedPoints; i++) {
                ...
                assertTrue(estEuclideanCam1.isPointInFrontOfCamera(pe));
                assertTrue(estdEuclideanCam2.isPointInFrontOfCamera(pe));
            }

            final var euclideanCenter1 = estEuclideanCam1.getCameraCenter();
            final var euclideanCenter2 = estdEuclideanCam2.getCameraCenter();
            final var euclideanCenter3 = estEuclideanCam3.getCameraCenter();

            final var euclideanIntrinsic1 = estEuclideanCam1.getIntrinsicParameters();
            final var euclideanIntrinsic2 = estdEuclideanCam2.getIntrinsicParameters();
            final var euclideanIntrinsic3 = estEuclideanCam3.getIntrinsicParameters();

            final var euclideanRotation1 = estEuclideanCam1.getCameraRotation();
            final var euclideanRotation2 = estdEuclideanCam2.getCameraRotation();
            final var euclideanRotation3 = estEuclideanCam3.getCameraRotation();
    ...
    void testGeneralPointsEssentialWithConstantAccelerationAndRotationWithoutNoiseFourViews()
        ...
        for (var t = 0; t < TIMES; t++) {
            ...
            final var center3 = new InhomogeneousPoint3D(
                    ...
                    center2.getInhomY() + cameraSeparation2,
                    center2.getInhomZ() + cameraSeparation2);
            final var center4 = new InhomogeneousPoint3D(
            ...
            for (var i = 0; i < numPoints2; i++) {
                ...
                // here 3D point is in front of 2nd camera
            ...
            var scaleTransformation = new MetricTransformation3D(baseline);
            final var rescaledCenter1 = scaleTransformation.transformAndReturnNew(metricCenter1);
            final var rescaledCenter2 = scaleTransformation.transformAndReturnNew(metricCenter2);
            final var rescaledCenter3 = scaleTransformation.transformAndReturnNew(metricCenter3);
            final var rescaledCenter4 = scaleTransformation.transformAndReturnNew(metricCenter4);
            ...
            scaleTransformation = new MetricTransformation3D(scale3);
            ...
            for (var i = start2; i < numPoints2; i++) {
                ...
                // check metric points
                final var rescaledPoint = Point3D.create();
                scaleTransformation.transform(metricReconstructedPoints3D.get(i - start2), rescaledPoint);
                ...
                // check that scale error is less than 5%
                if (Math.abs(scaleX - baseline / scale3) > ABSOLUTE_ERROR) {
                    continue;
                }
                assertEquals(scaleX, baseline / scale3, ABSOLUTE_ERROR);
                if (Math.abs(scaleY - baseline / scale3) > ABSOLUTE_ERROR) {
                    continue;
                }
                assertEquals(scaleY, baseline / scale3, ABSOLUTE_ERROR);
                if (Math.abs(scaleZ - baseline / scale3) > ABSOLUTE_ERROR) {
                    continue;
                }
                ...
                rescaledPoint.setInhomogeneousCoordinates(
                        rescaledPoint.getInhomX() * baseline / scale3,
            ...
            for (var i = 0; i < numPoints3; i++) {
                ...
                // check metric points
                final var rescaledPoint = Point3D.create();
                scaleTransformation.transform(metricReconstructedPoints3D.get(i + numPoints2 - start2), rescaledPoint);
    ...
    private static SlamCalibrator createFinishedCalibrator(
